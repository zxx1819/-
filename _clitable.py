...
import textfsm
from _textfsm import _texttable as texttable
...
class CliTable(texttable.TextTable):
  ...
  """
    ...
    def ParseCmd(self, cmd_input, attributes=None, templates=None):
        """从cmd_input字符串创建一个值的文本表。
            使用模板/s解析命令输出。如果找到多个模板
            如果键匹配(否则删除)，则合并后续表。
            参数:
            cmd_input:字符串，设备/命令响应。
            属性:Dict，进一步细化匹配模板的属性。
            模板:要解析的模板的字符串列表。如果没有，则使用索引
            提出了:
            CliTableError:未找到给定命令的模板.
    """
        # 在对象中存储原始命令数据.
        self.raw = cmd_input
